% main points of reducing:
% 20% of cells selected
% the order of repi and i is changed, and for each repi the 20%
% subselection is done. So effectively, instead of just randomizing splits
% for cross validation, repi randomly selects cells, so effectively the
% results are averaged over nrep selections of cells.
...
lmodel.filename = ['reduced20',lmodel.exptype,'lmodel_25Th_',lmodel.binsizems,'ms_X_style',num2str(lmodel.Xstyle),'_nrep',num2str(lmodel.nrep),'.mat'];
...
for animalnum= includedanimals
    ...
    if lmodel.eqtrials
        ...
        targetcell_i = equalizegonogotrials(targetcell,0,0,randeq);% frnormalize
        notargetcell_i = equalizegonogotrials(notargetcell,0,0,randeq);% frnormalize
    end
    ...
    for repi = 1:lmodel.nrep
        ...
        rng(repi,'twister')
        clear round
        num_red = round(0.2*numel(targetcell_i));
        targetcell = targetcell_i(randsample(numel(targetcell_i),num_red));
