%%% 
% - it is simplified, most com parts (lda) cut out
% - just activity matters. The rest (communication) is dummy
% - targetcells and notargetcells are selected based on ratio. 
% - change only subratio and exptype to make new files
% - RC criteria like the original lda files

% This scrpit and the corresponding X_cor_timeconstant_subactivityonly.m
% subselect V1 and LM cells based on their dynamics to show that 1 - they
% are not that different 2- even the fast ones dont have a large go/nogo
% difference.

% first run this, a dummy file is made. Then run
% X_cor_timeconstant_subactivityonly.m that accesses and loads these dummy
% file (dummy because only activity 65, no lda calculated)
...
lmodel.exptype = 'FB';
lmodel.binsizems = '65';
lmodel.subratio = 0.1; %if less than 0.5, more dynamic than usual, if more, less dynamic
lmodel.filename = ['dummyforactivity',num2str(lmodel.subratio),lmodel.exptype,'lmodel_25Th_',lmodel.binsizems,'ms_X_style',num2str(lmodel.Xstyle),'_nrep',num2str(lmodel.nrep),'.mat'];
...
for animalnum= includedanimals
    ...
    targetcell =  subselectdynamiccells(targetcell,lmodel.subratio); 
    ...
    [acmatgo,acmatnogo,~,~,~,~] = X_prepareCmatrices_activity_subdynamics(animalmodel,animalnum,lmodel,V1cells,LMcells,0,lmodel.subratio);% last one: normalizaation
    animalmodel{animalnum}.lmodel.acmatgo_n0 = acmatgo(:,:,animalnum);
    animalmodel{animalnum}.lmodel.acmatnogo_n0 = acmatnogo(:,:,animalnum);
    
    [acmatgo,acmatnogo,~,~,~,~] = X_prepareCmatrices_activity_subdynamics(animalmodel,animalnum,lmodel,V1cells,LMcells,1,lmodel.subratio);% last one: normalizaation
    ...
    [acmatgo,acmatnogo,~,~,~,~] = X_prepareCmatrices_notarget_activity_subdynamics(animalmodel,animalnum,lmodel,V1cells,LMcells,0,lmodel.subratio);% last one: normalizaation
    ...
    [acmatgo,acmatnogo,~,~,~,~] = X_prepareCmatrices_notarget_activity_subdynamics(animalmodel,animalnum,lmodel,V1cells,LMcells,1,lmodel.subratio);% last one: normalizaation
