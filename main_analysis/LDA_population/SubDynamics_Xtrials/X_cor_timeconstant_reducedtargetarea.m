...
% This loads the lda file made by X_lda_new_reducedtargerarea.m which is
% com directions using 20% of neurons in the target area. This script is
% mainly for saving tau values for analysis in X_tau_comparison. Most other
% outputs are not needed for nanything.
...
peranimal = 1;% if 1: calculates tau per animal as well as the combied one.
saveres = 1; % if 1: automatically makes timestamped folder and saves figures and tau estimates.
...
ldafilename = ['reduced20',exptype,'lmodel_25Th_',binsizems,'ms_X_style',xstyle,'_nrep',nrep,'.mat'];
...
for animalnum = allanimals
    ...
    if peranimal
        ...
        if 0 % skipped per animal plots to save time
...
if saveres
    cd(savdir)
    foldername = ['targetreduced20',exptype,binsizems,'Xstyle',xstyle,'nrep',nrep,'_',n1type,'_','ldaormu',num2str(ldaormu),...
